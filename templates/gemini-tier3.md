...
> 铁律：**能由 hook/CI 兜底的，模型不做**（写盘拦截/写后校验归 hook，validate 归 CI/收尾）；本地写盘后不自行复验；多条引擎命令一律用 `;` 拼进**同一次调用**。
...
```
> 省 token/省调用：用 `;` 批量把命令拼进**同一次调用**；汇总命令加 `--json` / `--quiet`；`validate` 只须在收尾/合并前跑一次（非阻塞告警无需每步处理）。**一键拼接模板见 `.asa/rules/call-minimization.md`（按需加载）。**
