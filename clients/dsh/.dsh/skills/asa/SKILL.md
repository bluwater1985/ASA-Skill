---
name: asa
description: AI Software Architect（ASA）—— 在项目中初始化 ASA 工程管理流程（DeepSeek Harness 版）。支持 Tier 1（探索验证）、Tier 2（离线防御）、Tier 3（强契约）三级渐进式启用。用户说「初始化 ASA」「asa init」或要求用 ASA 管理项目时触发，自动搭建 .asa/ 引擎目录、生成项目契约指令 GEMINI.md / CLAUDE.md、配置增量方法库与 pre-commit。
---
...
> **触发方式**：在 DeepSeek Harness 中，直接对助手说「初始化 ASA」/「asa init」，或要求「用 ASA 管理此项目」即可触发本技能。
...
优先使用 `node ~/.dsh/skills/asa/scripts/asa-init.js` 完成初始化。如果脚本不存在，按后续步骤手动搭建。

```bash
# 根据所选 Tier 运行（tier1 / tier2 / tier3）
node ~/.dsh/skills/asa/scripts/asa-init.js tier2

# 加 --force 可重新生成 GEMINI.md（旧文件自动备份）
node ~/.dsh/skills/asa/scripts/asa-init.js tier2 --force
```

初始化脚本会自动完成（**幂等**：已有文件不覆盖）：
- 创建 `.asa/` 目录结构和引擎文件
- `matrix.yaml` 和 `GEMINI.md` → 存在即跳过，不覆盖已有数据
- 复制增量方法库 `to-spec.md` / `to-tickets.md` / `to-issues.md` → `.asa/rules/`
...
#### 复制增量方法库规则（to-spec / to-tickets / to-issues）
```bash
mkdir -p .asa/rules
cp ~/.asa/rules/to-spec.md ~/.asa/rules/to-tickets.md ~/.asa/rules/to-issues.md .asa/rules/
```
...
| `add-issue <title> [--category <bug\|requirement-clarification\|observation\|risk>] [--severity P0-P3] [--desc <文件\|串>] [--task <id>] [--req <id>] [--arch <id>]` | 新增问题节点（Schema v4） | 第 4 类节点 `ISSUE-xxx`，默认 `observation / P2`。提出问题时**先分流**：确认 bug → 建修复 TASK；需求没写清 → 改/补需求文档；否则以 observation/risk 观察。`--desc` 写入详情（文件形式自动归档 `.asa/specs/<id>-issue.md`）；`--task/--req/--arch` 自动写 `affects` 边。 |
| `status ISSUE-xxx <状态>` | ISSUE 状态机推进 | `open→triaged→in_progress→resolved→verified`，另有 `cancelled/wontfix`。`→resolved` 须 `--note "<处置>"`；`resolved→verified`、`resolved→open/in_progress`、`cancelled→open` 须 `--by`；`verified` 为吸收终态不可回开。 |
...
所有节点标题在文档/清单中统一为 `<ID> - <名称>`（如 `REQ-001 - 用户登录`）。「名称」部分按类型遵循不同语义：

- **REQ（需求）**：**名词短语**，讲"能给什么能力/价值"，**不说怎么做**。如 `REQ-001 - 用户登录`、`REQ-012 - 支付对账能力`。
- **TASK（任务）**：**动词开头**，讲"做什么"、可验收。如 `TASK-003 - 实现登录接口`、`TASK-021 - 修复订单金额溢出`。
- **ISSUE（问题）**：讲"**现象 + 受影响场景**"，不预设修复（便于分流 bug / 澄清 / 观察）。如 `ISSUE-002 - 登录偶发500（手机端用户）`、`ISSUE-007 - 支付成功但订单仍显示未支付`。
- **ARCH（架构）**：**名词短语**，组件/抽象层名。如 `ARCH-002 - 支付网关抽象层`。

**铁律**：整仓同一语言（默认中文）；不带标点结尾；不带版本号/日期/实现细节；「名称」≤ 40 字（ISSUE 可略长）；ID 由引擎自动生成或 `--id` 指定，**无须手写编号**。
...
初始化时脚本会自动把 **to-spec（需求分析）**、**to-tickets（任务拆解）** 与 **to-issues（问题记录）** 三套增量方法复制到项目 `.asa/rules/`。这三套方法的完整规约**默认不加载**，由 GEMINI.md / CLAUDE.md 的「增量方法库」段兜底触发：
...
> 维护建议：如需调整这些方法的模板或流程，直接改 `.asa/rules/to-spec.md` / `to-tickets.md` / `to-issues.md`，无需改动 GEMINI.md 常驻指令。
