...
| `add-issue <title> [--category <bug\|requirement-clarification\|observation\|risk>] [--severity P0-P3] [--task <id>] [--req <id>] [--arch <id>]` | 新增问题节点（Schema v4） | 第 4 类节点 `ISSUE-xxx`，默认 `observation / P2`。提出问题时**先分流**：确认 bug → 建修复 TASK；需求没写清 → 改/补需求文档；否则以 observation/risk 观察。`--task/--req/--arch` 自动写 `affects` 边。 |
