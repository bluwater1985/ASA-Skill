...
const {
  ...
  clearPendingPropagation, hasPendingPropagation,
...
describe('appendChangeLog', () => {
  ...
  it('increments version for awaiting-confirmation status', () => {
    const node = { version: 1 };
    const v = appendChangeLog(node, 'awaiting-confirmation', '等待确认');
    assert.equal(v, 2);
    assert.equal(node.version, 2);
  });
});
