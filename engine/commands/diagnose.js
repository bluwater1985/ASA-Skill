...
function run() {
  io.info(`[ASA DIAGNOSE] 引擎版本: ${ENGINE_VERSION}`);
  let matrix;
  try {
    matrix = loadMatrix();
  } catch (e) {
  ...
  io.info(`[ASA DIAGNOSE] 项目 Schema 版本: ${sv}`);
  
  if (sv < MIN_SCHEMA_VERSION) {
    io.info(`[ASA DIAGNOSE] ⚠️ Schema 版本 (${sv}) 低于引擎最低要求 (${MIN_SCHEMA_VERSION})，请运行 reconcile 升级。`);
  }
  ...
  io.info(`[ASA DIAGNOSE] Nodes Digest (当前/预期): ${actualNodesDigest} / ${expectedNodesDigest || 'none'}`);
  if (actualNodesDigest !== expectedNodesDigest) {
    io.info('[ASA DIAGNOSE] ⚠️ Nodes 存在脏增量未入账。');
  }
  ...
  io.info(`[ASA DIAGNOSE] Docs Digest (当前/预期): ${actualDocsDigest} / ${expectedDocsDigest || 'none'}`);
  if (actualDocsDigest !== expectedDocsDigest) {
    io.info('[ASA DIAGNOSE] ⚠️ [DOCS_TAMPERED] 编译型文档 (docs) 与预期哈希不一致，可能是被外部直接修改了，请检查或重新 compile。');
  }
  ...
  const checkBasedOn = (filename) => {
    ...
    if (fs.existsSync(p)) {
      ...
      if (match) {
        io.info(`[ASA DIAGNOSE] ${filename} 基于 nodesDigest: ${match[1]}`);
        if (match[1] !== actualNodesDigest) {
          io.info(`[ASA DIAGNOSE] ⚠️ ${filename} 已经过期，请让 LLM 重新渲染。`);
        }
      } else {
        io.info(`[ASA DIAGNOSE] ⚠️ ${filename} 缺少 ASA-BASED-ON 标记。`);
      }
    }
  };
  ...
  for (const hook of hooks) {
    ...
    if (!fs.existsSync(p)) {
      io.info(`[ASA DIAGNOSE] ⚠️ 缺失 Hook: ${hook}`);
    }
  }
  ...
  if (hasLegacyEdges) {
    io.info('[ASA DIAGNOSE] ⚠️ 存在未指定 type 的旧版本 (Legacy) 边，请升级数据。');
  }
            ...
            if (raw) {
              try {
                ...
                if (manifest.status !== 'completed') {
                  io.info(`[ASA DIAGNOSE] ⚠️ 发现未提交的脏事务: ${item} (状态: ${manifest.status}，开始于: ${manifest.startedAt || 'unknown'})！为了保证数据安全，建议运行任一写命令或 reconcile 执行物理回滚自愈。`);
                }
              } catch (parseErr) {
                io.info(`[ASA DIAGNOSE] ⚠️ 发现损坏的未完成事务目录: ${item} (清单损坏)！物理备份现场已锁定保护，请联系架构师或手动修复/清理 "${txDir}"。`);
              }
            }
          }
        }
  ...
  if (io.jsonOut({
    ...
    schemaVersion: sv,
    phase: matrix.meta?.phase || null,
    activeTask: matrix.meta?.activeTask || null,
    awaitingConfirmation: 0,
    nodesDigestCurrent: actualNodesDigest,
    nodesDigestExpected: expectedNodesDigest || null,
    nodesDirty: actualNodesDigest !== expectedNodesDigest,
    docsDigestCurrent: actualDocsDigest,
    docsDigestExpected: expectedDocsDigest || null,
    docsTempered: actualDocsDigest !== expectedDocsDigest
  })) return;

  io.info('[ASA DIAGNOSE] ✅ 诊断完成（只读）。');
}
